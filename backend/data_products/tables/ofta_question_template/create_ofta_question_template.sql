CREATE TABLE IF NOT EXISTS ofta_prod.ofta_question_template (
    ...
    celebrity_id        UUID            REFERENCES ofta_prod.ofta_celebrity(id),
    celebrity_id_a      UUID            REFERENCES ofta_prod.ofta_celebrity(id),
    celebrity_id_b      UUID            REFERENCES ofta_prod.ofta_celebrity(id),
    ...
    CONSTRAINT chk_ofta_qt_celebs CHECK (
        ...
        OR (mode = 'WHO_OLDER' AND celebrity_id_a IS NOT NULL AND celebrity_id_b IS NOT NULL)
    )
);
...
CREATE INDEX IF NOT EXISTS idx_ofta_qt_celebrity   ON ofta_prod.ofta_question_template(celebrity_id);
